...
public class GetByIdRecipeTest : MyRecipeBookClassFixture
{
    ...
    public async Task Error_Recipe_Not_Found(string culture)
    {
        ...
        var recipeInvalidId = IdEncripterBuilder.Build().Encode(1000);

        var response = await DoGet(method: $"{METHOD}/{recipeInvalidId}", token: token, culture: culture);
